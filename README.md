...
CLI-инструмент, который анализирует код проектов из Git-репозиториев, ищет логические ошибки и уязвимости (без фокуса на стиль), сохраняет большие объемы кода в памяти (ChromaDB) и выводит результаты анализа.
...
- `model_api.py` — интеграция с OpenAI, язык/промпты, чанкинг и память.
...
```bash
export GIGACHAT_BASE_URL="https://gigachat.devices.sberbank.ru/api/v1/chat/completions"
export GROQ_BASE_URL="https://api.groq.com/openai/v1/chat/completions"
```
...
4. `model_api.py`, `gigachat_api.py` или `groq_api.py` для каждого файла:
